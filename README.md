...
- 🎯 Entry points to execute your program using `python -m <byzer_llm_api>` or `$ byzer_llm_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/MrFengJian/byzer-llm-api/branch/main/graph/badge.svg?token=byzer-llm-api_token_here)](https://codecov.io/gh/MrFengJian/byzer-llm-api)
[![CI](https://github.com/MrFengJian/byzer-llm-api/actions/workflows/main.yml/badge.svg)](https://github.com/MrFengJian/byzer-llm-api/actions/workflows/main.yml)

Awesome byzer_llm_api created by MrFengJian

## Install

from source
```bash
git clone https://github.com/MrFengJian/byzer-llm-api byzer_llm_api
cd byzer_llm_api
make install
```
...
```bash
pip install byzer_llm_api
```

## Executing

```bash
$ byzer_llm_api run --port 8080
```

or

```bash
python -m byzer_llm_api run --port 8080
```

or

```bash
$ uvicorn byzer_llm_api:app
```
...
```bash
❯ byzer_llm_api --help
Usage: byzer_llm_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ byzer_llm_api create-user --help
Usage: byzer_llm_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
byzer_llm_api create-user admin admin --superuser
```
...
Run with `byzer_llm_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `byzer_llm_api create-user`.
...
-----------------------------------------------------
byzer_llm_api/__init__.py              4      0   100%
byzer_llm_api/app.py                  16      1    94%
byzer_llm_api/cli.py                  21      0   100%
byzer_llm_api/config.py                5      0   100%
byzer_llm_api/db.py                   10      0   100%
byzer_llm_api/models/__init__.py       0      0   100%
byzer_llm_api/models/content.py       47      1    98%
byzer_llm_api/routes/__init__.py      11      0   100%
byzer_llm_api/routes/content.py       52     25    52%
byzer_llm_api/routes/security.py      15      1    93%
byzer_llm_api/routes/user.py          52     26    50%
byzer_llm_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in byzer_llm_api/default.toml.
